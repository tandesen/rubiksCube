...
| `badges.py` | 课程标题/记号框,支持定义、定理、证明、公理、命题和 notation 预设 |
...
`course_badge()` 是核心函数,可以接收文字或已组合好的 Manim
`Mobject`。预设包括 `definition` / `theorem` / `proof` / `axiom` /
`proposition`(命题) / `notation` / `move`。

```python
from rubikscube import course_badge, theorem_heading

theorem_heading("有限群元素", label="1.1")
...
course_badge(
    ...
    title="记号",
    label="A",
    fill_color="#264674",   # 所有颜色/尺寸都可按次覆盖
)
```

便捷函数还有 `def_heading()`、`theorem_heading()`、
`axiom_heading()`、`proposition_heading()`、`proof_heading()` 和
`notation_tag()`; 它们都调用同一个 `course_badge()`。
